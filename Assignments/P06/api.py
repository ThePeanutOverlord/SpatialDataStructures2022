...
dataPath = "C:/Users/champ/OneDrive/Documents/4553-Spatial-DS-Champagne/Assignments/P06/"
...
def countryCentroid(name):
    ...
    Returns:
        (tuple): point (x,y)
    """
    countryPolys = countryDB.getRawPolygons(name)
    print(countryPolys)

    # largest = largestPoly(countryPolys["geometry"]["coordinates"])
    center = centroid(countryPolys)
    return center
...
def countryPoly(country):
    ...
    # Go get the polygons
    polys = countryDB.getRawPolygons(country)
...
async def getCountryNames():
    ...
    """
    names = countryDB.getCountryNames()
    if names:
        return names
    else:
...
async def getCountry(country_name, coords_only: bool = False):
    ...
    # Go get the polygons
    polys = countryDB.getRawPolygons(country_name)
    ...
    f = Feature(coords=polys, properties={"name": country_name})
...
async def countryCenter(country_name, raw: bool = False):
    ...
    centers = []
    country = countryDB.getRawPolygons(country_name)
...
async def getRandomCountry():
    """ 
    [http://127.0.0.1:8080/randomCountry/]
    """
    names = countryDB.getCountryNames()
    shuffle(names)
    target = names[0]
    ...
    country = countryDB.getRawPolygons(target)
    # largest = largestPoly(country["geometry"]["coordinates"])

    # print(largest)
    center = centroid(country)
